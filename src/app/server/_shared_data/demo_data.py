...
NOTE: this is a vendored copy of src/shared/bids_demo/demo_data.py. A
Databricks App only deploys its own `source_code_path` (src/app here), so it
can't reach the sibling src/shared/ package used by the bootstrap jobs — this
copy exists so the app's in-memory fallback (local dev without Lakebase) has
the same data without a cross-directory import. Keep the two in sync if you
change the schema.
"""
